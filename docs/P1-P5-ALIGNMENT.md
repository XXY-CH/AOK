...
| 阶段 | 原始设想 | 当前实现 | 尚缺验收 |
| --- | --- | --- | --- |
| P1 | aproc、VTC/MLFQ、amem COW、IPC、/proc、监督恢复 | root/aproc/task/freezer/resource/event/registry 内核切片；PID1 引导；用户态 CAS/COW、mailbox/checkpoint 恢复 | kernel amem/LSFS、sched_ext Agent 调度、COW fan-out 接线、/proc 统计、guest 内监督重启演练、独立 fsd |
| P2 | ainf 同一 fd ABI、真实后端、vsock、token budget、亲和放置 | llama/Metal/Anthropic provider、router、usage/cache/亲和；0006 ainf 与 KernelInferProvider probe 闭环 | 生产 engine 接入 kernel-infer、vsock 心跳、AFM、cache-affinity spawn、多 slot 联合验证、同 fd ABI 多后端验收 |
| P3 | acapd、Biscuit/Cedar、内核能力门、污点、确认、审计/witness | 用户态 capability、taint gate、确认、撤销、审计 hash chain 和本地 witness；内核推理 capability/revoke | Rust acapd、Biscuit/Cedar、完整 Landlock/seccomp 编译及 kernel gates、unotify、外部 witness 服务 |
...
| P5 | ash 一条命令、COW researchers、三后端并行与工具、port 回传、LSFS、/proc 与审计 | shell 入口、实际 planner 文本传递、researcher/aggregator turns、持久报告；echo 与单 llama 数据流验证 | COW、真正并行、三后端、工具、port、完整 LSFS/fsd、ash、/proc 和公平性验收 |
...
- P5 消费真实 planner 输出，由 aggregator provider 生成报告；默认保留结果和 state，支持复用 supervisor。独立 application.create 尚不等于 COW fork。
...
- `make runtime-smoke runtime-core-smoke runtime-mvp-smoke`：引擎握手、crash replay、timer、LSFS 恢复及报告保留/重启读取通过。
- `make runtime-mvp-mixed-smoke`：真实 llama 单后端完整数据流通过；小模型 fixture 只验证接口与缓存，不证明研究报告质量或三后端混合。
- `make aok-event-probe-test aok-initfs-boot-test`：真实 QEMU 内核事件、supervisor 同 boot 重启、推理预算及 PID1 启动通过；启用 proc sysctl 后 appregistry 47、resource 56、core 46 项回归通过。
